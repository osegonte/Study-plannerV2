...
interface SimpleProgressSidebarProps {
  pdfId: string
  pdfData: any
}

export default function SimpleProgressSidebar({ pdfId, pdfData }: SimpleProgressSidebarProps) {
  const { state } = usePDF()
  ...
  const loadBasicStats = async () => {
    try {
      const statsData = await apiClient.request(`/reading/stats/${pdfId}`)
      setStats(statsData)
    } catch (error) {
  ...
  return (
    <div className="w-80 bg-white border-l border-gray-200 flex flex-col">
      ...
      <div className="p-4 border-b">
        ...
        <p className="text-sm text-gray-600">Stage 2: Basic Time Tracking</p>
      </div>
      ...
      <div className="flex-1 p-4 space-y-6 overflow-y-auto">
        ...
        {stats && (
          <div>
            <h4 className="font-medium text-gray-900 mb-3">Basic Statistics</h4>
        ...
        <div className="border-t pt-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-xs text-blue-800">
              <strong>Stage 2:</strong> Basic time tracking is active. 
              Stage 3 will add estimated reading times.
            </p>
          </div>
        </div>
      </div>
